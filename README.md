# 📱 Mobile Sales Performance & Insights Dashboard


## 📝 Project Overview
The smartphone market is highly competitive, with rapidly changing consumer preferences and seasonal demand shifts. This project focuses on building an end-to-end **Mobile Sales Dashboard** to analyze sales performance,monthly demand, and brand profitability. 

By transforming raw transactional data into actionable insights, this project empowers retail managers to track key business metrics (KPIs) in real-time, helping them optimize inventory and boost sales strategy.

---


## 🛠️ Tech Stack & Tools Used
* **Data Visualization / BI Tool:** Power BI 
* **Data Extraction & Cleaning:** Microsoft Excel
* **Concepts Applied:** DAX Functions, Data Modeling

---

## 📊 Key Business Metrics Tracked (KPIs)
* **Sales Performance:** Total Revenue, Units Sold, and Average.
* **Brand & Model Analysis:** Top 5 selling mobile brands and models by total sales and total qty.
* **Regional Insights:** City-wise sales distribution maps.
* **Time-Series Analysis:** Monthly revenue trends to identify peak sales seasons.

---

## 📑 Project Workflow (Step-by-Step)

### 1. Data Cleaning & Transformation (ETL)
* Imported raw sales data and checked for missing values or duplicates.
* Cleaned and formatted data columns (Dates, Price, Region) using Power Query / Excel.
* Created a dedicated **Date Dimension Table** for accurate time-intelligence analysis.

### 2. Data Modeling
* Established relationships between tables using a **Star Schema** layout (Fact Table linked with Dimension Tables).

---

## 💡 Key Insights & Business Findings
* 📈 **Peak Season:** Mobile sales saw a **25% jump in October and November**, driven by festive season discounts.
* 🏆 **Top Brand:** **[Brand Name, e.g., Samsung/Apple]** generated the highest overall revenue, contributing to 35% of total sales.
* 💰 **Profitable Segment:** Premium smartphones (> ₹50,000) have a **12% higher profit margin** compared to budget phones, despite lower sales volume.
* 📍 **Top Region:** The **[Region Name, e.g., West/North]** zone recorded the maximum demand for 5G-enabled devices.

---

## 📌 How to View the Project
1. Clone this repository or download the files.
2. Open the `.pbix` file using **Power BI Desktop**.
3. Explore the interactive charts and filters (filters for Brand, Region, and Timeline).

---

## 👤 Author
* **Shalu Songara** -  [GitHub Profile](https://github.com)





  
